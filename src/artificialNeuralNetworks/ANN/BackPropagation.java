...
public class BackPropagation {
    ...
    public static void learn (final NeuralNetwork net, final AnnExList exSet,
            ...
            for (AnnExample ex : exSet) {
                ...
                for (int layerId = net.layers.size() - 1; layerId >= 0; layerId--) {
                    updateError(net, layerId, ex.tList);
                }

                // Update weights in each layer.
                for (int layerId = net.layers.size() - 1; layerId >= 0; layerId--) {
                    updateWeights(net, layerId, ex.xList, learnRate, momentumRate);
                }
            } // End of for (AnnExample ex : exSet) {
    }

    private static void updateError (final NeuralNetwork net,
            final int layerId, final ArrayList<Double> train) {
